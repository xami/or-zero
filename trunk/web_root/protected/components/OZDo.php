	...
	'success'=>"function(data)
	{
		if(data==null){
			...
			$('#sinfo').addClass('sinfo').html(
				((data.responseStatus==301)?data.responseDetails+'<br />':'<input type=\"text\" class=\"ssrc\" size=\"60\" id=\"orzero_link\" value=\"推荐: '+
				data.responseData.link+' '+data.responseData.title+
				'(只看楼主,只看楼主)\" /><!--input type=\"button\" value=\"复制内容\" onClick=\"copy_link();\" class=\"bt_m so\" id=\"orzero_button\" /--><br />')+
				'<span class=\"grey\">标题:</span><a target=\"_blank\" href=\"'+data.responseData.link+'\"><span id=\"orzero_title\">'+data.responseData.title+
				'</span><span class=\"red\">(点此进入'+((data.responseStatus==220)?'阅读':'原帖')+')</span></a><br />'+
				'<span class=\"grey\">作者:</span><a target=\"_blank\" href=\"/search?cx=partner-pub-4726192443658314:lofclyqlq8w&cof=FORID:11&ie=UTF-8&un=or&q='+
				data.responseData.un+'\">'+
				data.responseData.un+'(所有文章)</a>&nbsp;'+
				'<span class=\"grey\">页数:</span>'+((data.responseData.page>100) ? '<span class=\"green\">' : '<span class=\"wd red\">')+
		...
		$cs->registerScript('open_snoti', '$( "#open_snoti" ).click(function() {$( "#snoti" ).dialog( "open" );return false;});', CClientScript::POS_READY);
		...
		然后点击<span class="green">[只看楼主]</span>,
		...
		echo CHtml::button('只看楼主', array( 'id'=>'so','class'=>'so bt_m','onclick'=>"{orzero();}" ) ); 
		echo '</div>';
...
目前支持<a target="_blank" href="http://www.tianya.cn/bbs/index.shtml"><span class="grey">[天涯论坛]</span></a>
