...
//		$bgcolor=ImageColorAllocate($watermark, 0, 0, 0);
//		ImageColorTransparent($watermark, $bgcolor);
...
//        $font = imageloadfont(Yii::getPathOfAlias('application.data.tianya.font').DIRECTORY_SEPARATOR.'dandelion_in_the_spring.ttf');
		...
		// Merge the images
		if ($status = imagecopymerge($this->tmp_image, $watermark, $width-110, $height-20, 0, 0, imagesx($watermark), imagesy($watermark), 100))
		{
			// Prevent the alpha from being lost
			imagealphablending($this->tmp_image, TRUE);
			imagesavealpha($this->tmp_image, TRUE);
